    ...
    getPuzzleByDifficulty() {
        switch (this.state.levelDifficulty) {
            case Difficulty.Easy:
                return <Board bigMinesCount={1} mediumMinesCount={2} smallMinesCount={3} levelHeight={7} levelWidth={7} onRedirect={(pv)=>this.onRedirect(pv)}/>;
            case Difficulty.Medium:
                return <Board bigMinesCount={3} mediumMinesCount={5} smallMinesCount={7} levelHeight={10} levelWidth={10} onRedirect={(pv)=>this.onRedirect(pv)} />;
            case Difficulty.Hard:
                return <Board bigMinesCount={6} mediumMinesCount={10} smallMinesCount={14} levelHeight={15} levelWidth={15} onRedirect={(pv)=>this.onRedirect(pv)} />;
            default:
                return <div></div>;
        }
    }
